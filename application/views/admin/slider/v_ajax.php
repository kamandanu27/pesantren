<!-- Data table plugin-->
   ...
   <script type="text/javascript">
      ...
      $(document).on( "click",".btn_del_foto", function() {
			...
			{
				type: 'GET',
				url : "<?= base_url() ?>admin/slider/delete_idfoto/"+id,
				data: 'id=' + id,
				dataType: 'json',
				success: function(data, textStatus, jqXHR)
				{
      ...
      var tabel_foto = $('#tabel_foto').DataTable({
			...
			"order": [],
			"ajax": {
				"url": "<?= base_url()?>admin/slider/tabel_foto/"+$("#id_slider").val(),
				"type": "GET"
			},
			"columnDefs": [{
				"targets": [0],
      ...
      //save dermaga
		$(document).on( "click",".btn_save_foto", function() {
         ...
         data.append('id_slider', $("#id_slider").val()); 
         $.ajax({
         url: '<?= base_url()?>admin/slider/insert_foto',
         type: 'POST',
         data: data,
         cache: false,
         processData: false,
