<!-- Data table plugin-->
   ...
   <script type="text/javascript">
      ...
      $(document).on("click",".btnhapus",function(){
         ...
         var value = {
            ...
            closeOnConfirm: true
               ...
               $.ajax(
               {
                  url : "<?=base_url()?>admin/brosur/delete",
                  type: "POST",
                  data : value,
                  success: function(data, textStatus, jqXHR)
                     ...
                     var data = jQuery.parseJSON(data);
                        if(data.result ==1){
                           ...
                           window.location= '<?=base_url()?>admin/brosur/';
                           alerthapus();
      ...
      $(document).on("click",".btnstatus",function(){
         ...
         var value = {
            ...
            closeOnConfirm: true
               ...
               $.ajax(
               {
                  url : "<?=base_url()?>admin/brosur/update_status",
                  type: "POST",
                  data : value,
                  success: function(data, textStatus, jqXHR)
                     {
      ...
      $(document).on( "click",".btn_del_foto", function() {
			...
			{
				type: 'GET',
				url : "<?= base_url() ?>admin/brosur/delete_idfoto/"+id,
				data: 'id=' + id,
				dataType: 'json',
				success: function(data, textStatus, jqXHR)
				{
      ...
      var tabel_foto = $('#tabel_foto').DataTable({
			...
			"order": [],
			"ajax": {
				"url": "<?= base_url()?>admin/brosur/tabel_foto/"+$("#id_brosur").val(),
				"type": "GET"
			},
			"columnDefs": [{
				"targets": [0],
      ...
      //save dermaga
		$(document).on( "click",".btn_save_foto", function() {
         ...
         data.append('id_brosur', $("#id_brosur").val()); 
         $.ajax({
         url: '<?= base_url()?>admin/brosur/insert_foto',
         type: 'POST',
         data: data,
         cache: false,
         processData: false,
