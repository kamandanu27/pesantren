...
class Berita extends CI_Controller {
	...
	public function view($status='')
	{
		if($status == 'request'){
			...
			$status_new = 'Unpublish';
		}else{
			$status_new = 'Draft';
		}
		$data = array(
			'title'			=> 'Berita - '.$status_new,
			'judul'			=> 'Berita - '.$status_new,
			'status'		=> $status_new,
			'content'		=> 'admin/berita/v_content',
			'ajax'	 		=> 'admin/berita/v_ajax'
		);
	...
	public function tabel($status='')
	{
		...
		$cek = $this->blog->tabel_berita($status)->result();
		$data = array();
		$no=1;
		foreach ($cek as $r) {
			$row = array();
			...
			$row[] = "<img src='../../public/image/upload/berita/$blog_gambar' style='width:100px;'>";
			$row[] = $blog_judul;
			$row[] = $name_user;
			$row[] = $tgl_riwayat;
			$row[] = $blog_views;
			...
			}elseif($blog_status == 'Reject' or $blog_status == 'Unpublish'){
	...
	public function insert()
	{
		...
		$config['upload_path'] 				= './public/image/upload/berita/';
		...
		redirect(base_url('admin/berita/draft'),'refresh');
	...
	public function update()
	{
		...
		redirect(base_url('admin/berita/'.$alamat),'refresh');
	}
	...
	public function update_publis_user()
	{
		...
		if($valid->run()==false){
			$this->session->set_flashdata('flash', 'Eror Tidak ada');
			redirect(base_url('admin/berita/publish'),'refresh');
		}else{
